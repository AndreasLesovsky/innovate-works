<?php
require("includes/bilder.inc.php"); //enthält Funktion "erzeugeQuadratischenAusschnitt"
...
if (count($_POST) > 0) {
	...
	foreach ($userDaten as $key => $value) {
		$userDatenTxt .= $key . ': ' . $value . PHP_EOL;
	}
	...
	if (isset($_FILES['dokumente'])) {
		foreach ($_FILES['dokumente']['tmp_name'] as $key => $tmpName) {
			...
			} else if ($_FILES['dokumente']["error"][$key] != 4) { // Error 4 bedeudet es hat kein Upload sattgefunden, in Ordnung für ein input[type="file"] Element ohne required Attribut
	...
	if (empty($errors)) {
		$msg = '<div class="success">
                    ...
                    Das Speichern Ihrer Daten war erfolgreich. Sie haben ' . $docCount . ' Dokument(e) hochgeladen.
                </div>';
	} else {
		$msg = '<div class="error">
                    <span>
						...
						Leider sind beim Hochladen Ihrer Daten einer oder mehrere Fehler aufgetreten:
                    </span>
	...
	<main>
		...
		<form id="upload-form" method="post" enctype="multipart/form-data">
			...
			<fieldset>
				...
				<p>Bitte beachten Sie dass nur das PDF Format für Dokumente unterstützt wird und die Gesamtgröße aller Dateien 10 MB nicht überschreiten darf.</p>
			</fieldset>
			<fieldset>
